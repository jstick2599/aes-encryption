...
void populateInitialMatrix(vector<vector<int>>&mass, unsigned char normal[]){//pass vector by reference
    int k=0; 
     mass.resize(4); 
     for(int i=0; i<4; i++){ 
        mass[i].resize(4); 
...
int main(){
    ...
    //lines 365-368 is the last round of aes encryption which includes everything but mix columns
       for(int i=0; i<4; i++){
            for(int j=0; j<4; j++){
                cout<<hex<<bite[i][j]<<" "; //prove that message has been encrypted
                if(j==3){
                    cout<<endl; 
                }
            }
